...
pub trait Handler {
    ...
    /// The application reports whether it handled a key event that carried a report id
    /// (Unbroken extension): CSI ? id ; handled u.
    fn report_key_event_handled(&mut self, _id: u16, _handled: bool) {}
...
bitflags! {
    ...
    pub struct KeyboardModes : u8 {
        ...
        /// Allow applications to handle deferred terminal keybindings (Unbroken extension).
    ...
    T: Timeout,
{
    ...
    fn csi_dispatch(
        ...
        match (action, intermediates) {
            ...
            ('u', [b'?']) => {
                // With parameters this is not the kitty keyboard mode query but an application's
                // key event handling report (Unbroken extension): CSI ? id ; handled u, sent in
                // response to a key event that carried a report id.
                let mut handling_params = params.iter();
                match (handling_params.next(), handling_params.next()) {
                    (Some(id), Some(handled)) => {
                        handler.report_key_event_handled(id[0], handled[0] != 0)
                    },
                    _ => handler.report_keyboard_mode(),
                }
            },
            ('u', [b'=']) => {
...
mod tests {
    ...
    struct MockHandler {
        ...
        keyboard_mode_apply: KeyboardModesApplyBehavior,
        key_event_handled: Option<(u16, bool)>,
    }
    ...
    impl Handler for MockHandler {
        ...
        fn set_keyboard_mode(&mut self, mode: KeyboardModes, apply: KeyboardModesApplyBehavior) {
            ...
            self.keyboard_mode_apply = apply;
        }

        fn report_key_event_handled(&mut self, id: u16, handled: bool) {
            self.key_event_handled = Some((id, handled));
        }
    ...
    impl Default for MockHandler {
        fn default() -> MockHandler {
            MockHandler {
                ...
                keyboard_mode_apply: KeyboardModesApplyBehavior::Replace,
                key_event_handled: None,
            }
        }
    }

    #[test]
    fn parse_key_event_handled_report() {
        let mut parser = Processor::<TestSyncHandler>::new();
        let mut handler = MockHandler::default();

        // Without parameters this is the kitty keyboard mode query.
        parser.advance(&mut handler, b"\x1b[?u");
        assert!(handler.keyboard_mode_reported);
        assert_eq!(handler.key_event_handled, None);

        // With parameters it is the key event handling report.
        parser.advance(&mut handler, b"\x1b[?42;1u");
        assert_eq!(handler.key_event_handled, Some((42, true)));

        parser.advance(&mut handler, b"\x1b[?43;0u");
        assert_eq!(handler.key_event_handled, Some((43, false)));
    }

    #[test]
    fn parse_key_event_handling_keyboard_mode() {
        let mut parser = Processor::<TestSyncHandler>::new();
        let mut handler = MockHandler::default();

        parser.advance(&mut handler, b"\x1b[=128;2u");
        assert_eq!(handler.keyboard_mode, KeyboardModes::REPORT_KEY_EVENT_HANDLING);
        assert!(matches!(handler.keyboard_mode_apply, KeyboardModesApplyBehavior::Union));
    }
